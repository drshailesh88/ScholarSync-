...
| **OpenAlex (lexical + `search.semantic` dense)** | **Required-ish (default)** | Lexical recall + **dense semantic (GTE-Large) recall lane**; citation counts (S2-independent), OA links, concepts; **batch citation/PMID/DOI backfill** by id | `OPENALEX_API_KEY` *(now required — see note)* | Yes (circuit breaker + pacing) | Degrades to keyless (heavily 429-throttled, high latency); citation/semantic signal weakens. Surfaced via `sourceStatuses.{openalex,openalex_semantic}`. |
