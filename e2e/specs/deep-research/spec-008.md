...
- [ ] FAIL: The page renders no source-selection control.
...
- [ ] FAIL: Mode cards do not render the mode descriptions from `RESEARCH_MODES.description`.
...
- [ ] FAIL: The start button has no spinner, disabled-loading state, or label change during plan generation.
#### Plan Preview
...
- [ ] FAIL: The current preview component does not resync its local perspective copy if parent data changes after mount.
...
- [ ] FAIL: Query delete buttons render only when that perspective currently has more than one query.
