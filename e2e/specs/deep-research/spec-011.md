...
- [ ] FAIL: Open-access tooltip text is exactly `OA`.
...
- [ ] FAIL: Citations-panel entries are buttons, not direct links.
- [ ] FAIL: Citations-panel entries show title, authors, journal/year, and an evidence label; they do not render DOI, PubMed, PDF, or OA links inline.
...
- [ ] FAIL: Topic sanitization for all client-side downloads replaces non-alphanumeric characters with `_` and truncates the topic slug to 50 characters.
...
- [ ] FAIL: PDF export success downloads `{topicSanitized}_report.pdf`.
...
- [ ] FAIL: Copy success sets `copied = true` and clears it after exactly 2000 ms.
...
- [ ] FAIL: BibTeX export downloads `{topicSanitized}_references.bib`.
