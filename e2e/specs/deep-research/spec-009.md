...
- [x] PASS: The `handlers.onError` callback exists in the reader signature but is never invoked by the switch statement.
...
- [x] PASS: `executeResearch()` posts `POST /api/deep-research/execute` with JSON `{ topic: topic.trim(), mode, perspectives: confirmedPerspectives }`.
...
- [x] PASS: After the stream closes, the page still runs `setPageState(prev => prev === "running" ? "done" : prev)` as a fallback.
...
- [x] PASS: The progress-bar percentage text is `Math.round(progress)%`.
...
- [x] PASS: The done-state metadata line renders `{report.mode} mode · {report.totalSources} sources analyzed`.
