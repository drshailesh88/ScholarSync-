...
- [x] PASS: `page.tsx` defines `streamingSections`, renders streaming `ResearchDocument` previews, and animates them with an 800 ms timeout, but the execute API never emits `section` SSE events, so this path never runs.
- [x] PASS: The `section` branch in the shared SSE parser exists, but no currently used deep-research server route sends `type: "section"`.
- [x] PASS: The `_streamingMarkdown` variable in `page.tsx` is computed and never used.
- [x] PASS: The page passes `onError` into `readSSEStream()` during plan generation, but the reader never calls `handlers.onError`.
- [x] PASS: The preview component supports `isRegenerating`, but the page never passes that prop, so the spinner/disabled path is dead from the live route.
- [x] PASS: `SaveToLibraryButton` supports `isLoggedIn = false`, but the page never passes a false value, so the logged-out tooltip/disabled branch is dead from the live route.
- [x] PASS: `ProgressStage.status = "error"` exists in the component contract, but the `/deep-research` page never constructs an error-status stage.
- [x] PASS: The legacy single-endpoint route `POST /api/deep-research` still exists, but the current `/deep-research` page never calls it.
- [x] PASS: Even if `section` SSE events were added later, the current streaming-preview path would render them with `sources = []` because `report` is still `null` during the running state.
#### Execute Route Stage Mapping (`src/app/api/deep-research/execute/route.ts`)
- [x] PASS: `STAGE_MAP` maps engine stages `validating`, `generating-perspectives`, `building-tree`, and `searching` to the single frontend stage `search-round-1`.
- [x] PASS: Engine stage `search-round-3` maps to frontend `search-round-2` (no separate round-3 indicator on the frontend).
- [x] PASS: Engine stages `deduplicating` and `unpaywall-lookup` both map to frontend `full-text-extraction`.
- [x] PASS: Engine stage `synthesizing` maps to `synthesis-perspectives`; engine stage `complete` maps to `synthesis-critique`.
