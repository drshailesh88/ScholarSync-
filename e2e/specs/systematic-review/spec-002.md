# systematic-review — Spec 002

STATUS: PARTIAL
TESTED: 35/35
PASS: 16
FAIL: 19
BLOCKED: 0
PAGE: http://localhost:3001/systematic-review
MODULE: systematic-review

---
### Workflow Page — Layout & Navigation
#### Wrapper
- [x] PASS: **Project ID** — extracted from URL params and used to fetch/hydrate state
#### Navigation
- [x] PASS: **Back link** — "All Reviews" text link navigates back to `/systematic-review`
- [ ] FAIL: **Back link position** — top-left of page
- [x] PASS: **Browser back** — back navigation returns to hub page

### Workflow Page — Collaborator Presence
#### Online Indicators
- [x] PASS: **CollaboratorPresence component** — renders in workflow page header
- [x] PASS: **Avatar display** — shows online collaborator avatars
- [ ] FAIL: **Tooltips** — hover shows collaborator name, current tab, and current paper
- [ ] FAIL: **WiFi status indicator** — shows connection status icon
- [ ] FAIL: **Real-time updates** — avatars appear/disappear as collaborators join/leave
#### Collaborator API
- [x] PASS: `GET /api/systematic-review/collaborators` — fetches team members
- [x] PASS: `POST /api/systematic-review/collaborators` — adds a collaborator
- [x] PASS: `PUT /api/systematic-review/collaborators` — updates collaborator role/permissions
- [x] PASS: `DELETE /api/systematic-review/collaborators` — removes a collaborator

### Workflow Page — Project Header & Stage Stepper
#### Project Header
- [ ] FAIL: **Title** — project title displayed in header
- [x] PASS: **Subtitle** — "PRISMA 2020-compliant systematic review"
- [ ] FAIL: **Paper count badge** — shows total paper count in a pill/badge
#### 7-Stage Stepper
- [ ] FAIL: **Active stage** — visually highlighted (filled/colored)
- [ ] FAIL: **Completed stages** — show completion indicator
- [ ] FAIL: **Future stages** — shown as inactive/dimmed
- [x] PASS: **Connectors** — lines or arrows between steps
- [x] PASS: **Stage updates** — stepper reflects `reviewStage` from store

### Workflow Page — Tab System (15 Tabs)
#### Tab Bar
- [ ] FAIL: **Horizontal scrollable** — tabs arranged in a scrollable horizontal bar
- [ ] FAIL: **Active tab** — visually highlighted
- [ ] FAIL: **Tab icons** — each tab has a unique Phosphor icon
- [ ] FAIL: **Tab labels** — descriptive text label on each tab
- [ ] FAIL: **Click** — switches active panel content
#### Tab Registry
- [x] PASS: **Tab persistence** — `activeTab` stored in Zustand and survives page refresh
- [ ] FAIL: **Default tab** — first tab selected on initial load
- [ ] FAIL: **All 15 tabs** — verify all 15 tabs render in the bar

### Search Strategy Panel
#### PICO Input Form
- [ ] FAIL: **Form layout** — 4 labeled input fields for P, I, C, O
- [ ] FAIL: **Comparison marked optional** — visual indicator that C is not required
- [ ] FAIL: **All fields editable** — free-text input for each PICO element
#### Generate Search Strategy
- [x] PASS: **"Generate Search Strategy" button** — triggers strategy generation
- [x] PASS: **API call** — `POST /api/systematic-review/search-strategy` with PICO data
- [x] PASS: **Loading state** — button shows spinner during generation
