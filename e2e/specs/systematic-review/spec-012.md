...
- [ ] FAIL: Stage badge falls back to raw `reviewStage` text with neutral styling for unknown stage values
- [ ] FAIL: Paper-count text always renders as `{project.paperCount} papers`
...
- [ ] FAIL: Successful workflow load additionally fetches `/api/systematic-review/projects` to derive `paperCount`
- [ ] FAIL: Failure of the secondary projects fetch is tolerated silently without replacing the main workflow page
- [x] PASS: Workflow loading state is spinner-only and does not show loading copy
- [ ] FAIL: Route-level loading component shows skeleton icon, title bar, 5 skeleton tabs, and one large content skeleton
- [ ] FAIL: Route-level error component title is `Systematic Review unavailable`
- [ ] FAIL: Route-level error component message is `We couldn't load the systematic review tool. Please try again.`
- [x] PASS: Top-left back link text is exactly `All Reviews`
- [ ] FAIL: Back link uses a `Link` to `/systematic-review`
...
- [ ] FAIL: Workflow tab bar does not render tab icons in the current live shell
- [ ] FAIL: Workflow tab bar does not render tab counts in the current live shell
...
- [ ] FAIL: Self tooltip shows the active-tab label when `self.presence.activeTab` exists
