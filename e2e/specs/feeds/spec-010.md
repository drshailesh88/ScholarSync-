...
- [ ] FAIL: The page-level header always marks all visible feeds read globally; there is no page-level picker for a specific-feed mark-all-read action.
...
- [ ] FAIL: Import/export feedback uses `alert(...)` or silent failure, not toast notifications.
...
- [ ] FAIL: Unread text singular form: "1 unread article" (no trailing "s" when `totalUnread === 1`)
- [x] PASS: Export button icon: `Export` from phosphor-icons (not Download)
- [ ] FAIL: Export button label text: "Export" (visible inline text, not tooltip-only)
- [x] PASS: Import button icon: `Upload` from phosphor-icons
- [ ] FAIL: Import button label text: "Import" (visible inline text, not tooltip-only)
- [x] PASS: Mark all read icon: `Checks` from phosphor-icons
- [ ] FAIL: Mark all read label text: "Mark all read" (visible inline)
