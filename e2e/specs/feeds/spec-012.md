...
- [ ] FAIL: ArticleCard root element is a `<button>` (not div)
...
- [ ] FAIL: Favicon fallback: `article.feedSourceFaviconUrl || getFaviconUrl(article.feedSourceSiteUrl ?? article.link ?? "")`
...
- [ ] FAIL: Star button title attribute: "Unstar" when starred, "Star" when not
- [x] PASS: Star icon color when starred: `text-yellow-500 hover:text-yellow-400` (text color only, no background)
- [ ] FAIL: Save button title attribute: "Saved" when saved, "Save to Library" when not
...
- [ ] FAIL: DOI link text: "DOI" (not the actual doi value)
...
- [ ] FAIL: List view root element is a `<button>`
