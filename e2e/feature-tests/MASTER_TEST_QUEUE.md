...
| 595 | LaTeX | Version History — Snapshot-based — save/restore full file snapshots | /latex/[projectId] | ✅ PASS | The new History drawer now saved a file snapshot, restored it back into the active editor, and kept the file/store state aligned. | `64a9304` |
| 596 | LaTeX | Version History — Load versions from server via /api/latex/versions | /latex/[projectId] | ✅ PASS | The verified browser run hit `GET /api/latex/versions?fileId=...` and rendered the returned snapshot row in the drawer. | `64a9304` |
| 597 | LaTeX | Version History — Restore version — replace current content with snapshot | /latex/[projectId] | ✅ PASS | Restoring the saved version replaced the edited source with the earlier `Version one snapshot.` content in the live editor. | `64a9304` |
| 598 | LaTeX | Version History — Delete old versions — clean up | /latex/[projectId] | ✅ PASS | Deleting the saved snapshot removed it from the drawer after the confirmed `DELETE /api/latex/versions/[id]` request completed. | `64a9304` |
| 599 | LaTeX | Version History — Timestamp tracking — each version stamped with date/time | /latex/[projectId] | ✅ PASS | Saved snapshots render a human-readable timestamp in the History drawer, backed by the API `createdAt` value. | `64a9304` |
| 600 | LaTeX | Collaboration (Real-time) / YjsCollaborationProvider — CRDT-based — conflict-free concurrent editing | /latex/[projectId] | ⚠️ PARTIAL | The workspace is still wrapped in `YjsCollaborationProvider`, but no live multi-client CRDT session could be exercised because no collaboration WebSocket URL is configured in this environment. |  |
| 601 | LaTeX | Collaboration (Real-time) / YjsCollaborationProvider — WebSocket connection — real-time sync | /latex/[projectId] | ⚠️ PARTIAL | The top bar correctly stayed in the disconnected `Offline` state, but there was no `NEXT_PUBLIC_COLLABORATION_WS_URL` to verify a real socket connection or sync traffic. | `64a9304` |
| 602 | LaTeX | Collaboration (Real-time) / YjsCollaborationProvider — Connection status tracking — connected/disconnected states | /latex/[projectId] | ✅ PASS | The top bar now surfaces collaboration status and rendered the disconnected `Offline` pill on the live LaTeX page. | `64a9304` |
| 603 | LaTeX | Collaboration (Real-time) / Collaboration Cursors — Remote cursor display — see other users' cursor positions | /latex/[projectId] | ⚠️ PARTIAL | Remote cursor UI is still not rendered in the active workspace, so no second-user cursor could be displayed in-browser. |  |
| 604 | LaTeX | Collaboration (Real-time) / Collaboration Cursors — Color-coded — each collaborator has a unique color | /latex/[projectId] | ⚠️ PARTIAL | Collaborator colors exist in the provider helpers, but without rendered remote cursors or a live peer session this row remains unverified in the workspace UI. |  |
| 605 | LaTeX | Collaboration (Real-time) / Collaboration Cursors — Real-time tracking — cursors move as users type | /latex/[projectId] | ⚠️ PARTIAL | Real-time cursor motion could not be exercised because the rendered LaTeX workspace still lacks the remote cursor overlay path. |  |
| 606 | LaTeX | Collaboration (Real-time) / Collaborator Awareness — User presence — see who is currently editing | /latex/[projectId] | ⚠️ PARTIAL | Presence is tracked in the provider, but no live collaborator session was available to render another active editor in this environment. |  |
| 607 | LaTeX | Collaboration (Real-time) / Collaborator Awareness — Avatars in top bar — collaborator profile pictures/initials | /latex/[projectId] | ⚠️ PARTIAL | `CollaboratorAvatars` is mounted in the top bar, but with collaboration offline there was no peer avatar/initial to verify live. | `64a9304` |
| 608 | LaTeX | Collaboration (Real-time) / Collaborator Awareness — Typing status — indication when others are typing | /latex/[projectId] | ⚠️ PARTIAL | `TypingIndicator` is now mounted, but there was no connected collaborator session to drive a visible typing state. | `64a9304` |
| 609 | LaTeX | Collaboration (Real-time) / Collaborator Awareness — Selection awareness — see what others have selected | /latex/[projectId] | ⚠️ PARTIAL | Selection awareness is tracked in the provider state, but no selection overlay is rendered in the active LaTeX workspace. |  |
| 610 | LaTeX | Citation System / Citation Search (Cite Tab) — Search PubMed and Semantic Scholar databases | /latex/[projectId] | ✅ PASS | The Cite tab issued a live `GET /api/search/unified` request and returned cross-source paper results in the panel. |  |
| 611 | LaTeX | Citation System / Citation Search (Cite Tab) — Results show author, title, year | /latex/[projectId] | ✅ PASS | The returned Cite-tab results rendered title plus compact author/year metadata exactly in the search list. |  |
| 612 | LaTeX | Citation System / Citation Search (Cite Tab) — Up to 10 results per search | /latex/[projectId] | ✅ PASS | The Cite tab capped the returned paper list to the first ten results from the unified search flow. |  |
| 613 | LaTeX | Citation System / Citation Insertion Flow — Search for paper in Cite tab | /latex/[projectId] | ✅ PASS | Searching from the Cite tab returned live paper cards and allowed direct insertion from the same panel. |  |
| 614 | LaTeX | Citation System / Citation Insertion Flow — Click to insert — dispatches latex:insert-bibtex event | /latex/[projectId] | ✅ PASS | Clicking a Cite-tab result triggered the insertion flow and drove the workspace `latex:insert-bibtex` handler end to end. |  |
| 615 | LaTeX | Citation System / Citation Insertion Flow — Auto-generates BibTeX entry with proper formatting | /latex/[projectId] | ✅ PASS | The created `references.bib` file contained a structured `@article{...}` entry with author, title, journal, and year fields from the selected result. |  |
| 616 | LaTeX | Citation System / Citation Insertion Flow — If no .bib file exists — creates references.bib with the entry | /latex/[projectId] | ✅ PASS | After deleting the default bibliography file, inserting a citation recreated `references.bib` and populated it with the selected BibTeX entry. |  |
| 617 | LaTeX | Citation System / Citation Insertion Flow — If .bib file exists — appends entry to existing file | /latex/[projectId] | ✅ PASS | Dispatching a second citation into an existing bibliography appended a second `@article{...}` block instead of replacing the file. |  |
