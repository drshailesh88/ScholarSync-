...
// Mock canvas getContext for jsdom (which lacks native canvas support).
// This must run before any library code attempts to create a 2D context.
const _origGetContext = HTMLCanvasElement.prototype.getContext;
// @ts-expect-error -- overriding getContext with a simplified mock
HTMLCanvasElement.prototype.getContext = function (contextId: string, ...args: unknown[]) {
  if (contextId === '2d') {
    ...
    return {
      ...
      fillText: () => {},
      strokeText: () => {},
      measureText: () => ({ width: 0, actualBoundingBoxAscent: 0, actualBoundingBoxDescent: 0, fontBoundingBoxAscent: 0, fontBoundingBoxDescent: 0 }),
  ...
  return _origGetContext.call(this, contextId, ...args);
};
import {
  Ellipse,
  Path as FabricPath,
