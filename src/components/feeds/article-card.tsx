...
export function ArticleCard({ article, isSelected, onClick, onCite, onAI }: ArticleCardProps) {
  ...
  const snippet = article.abstractSnippet
    ? article.abstractSnippet.length > 120
      ? article.abstractSnippet.slice(0, 120) + "..."
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isSelected
          ? "bg-surface-raised border-border-subtle ring-1 ring-brand/20"
    ...
    >
      ...
      <h3
        className={cn(
          ...
          article.isRead ? "text-ink-muted font-normal" : "text-ink font-semibold"
        )}
      >
        {article.title}
      </h3>
      ...
      {/* Authors */}
      {article.authors && (
        <p className="text-xs text-ink-muted truncate mb-1">{article.authors}</p>
      )}

      {/* Snippet */}
      {snippet && (
        <p className="text-xs text-ink-muted/70 line-clamp-2">{snippet}</p>
      )}
