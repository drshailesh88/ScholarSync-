...
import { verifyNotebookSharePassword } from "@/lib/actions/notebook-share";
import { SharedNotebookViewer } from "./SharedNotebookViewer";
...
export function NotebookPasswordGate({
  token,
  notebook,
}: NotebookPasswordGateProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const valid = await verifyNotebookSharePassword(token, password);
      if (valid) {
        setUnlocked(true);
      } else {
    ...
    <div className="min-h-screen bg-[#020617] flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-[#0f172a] border border-white/8 rounded-xl p-8">
        <div className="flex flex-col items-center gap-4 mb-6">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-[#94a3b8] mt-1">
              Enter the password to view this notebook.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? "Verifying..." : "View Notebook"}
          </button>
        </form>
